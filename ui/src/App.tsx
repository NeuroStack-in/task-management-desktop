...
export default function App() {
  ...
  const devBar = USE_MOCK ? <DevBar onChange={() => setScenarioKey((k) => k + 1)} /> : null;

  // Auth gates everything, ahead of consent: every command the panel calls needs a session, so
  // polling the core before sign-in would only produce errors behind the consent gate.
  let body: ReactNode;
  if (!auth.ready) {
    body = (
    ...
    body = <LoginCard onSignedIn={auth.setStatus} />;
  } else {
    // Remounting on sign-in drops any snapshot polled for the previous user.
    body = (
      <Panel
        key={`${scenarioKey}:${auth.status.username ?? ""}`}
        devBar={devBar}
  ...
  return (
    <div className="flex h-full flex-col p-3.5">
      {auth.notice && (
        <div className="mb-2.5 flex shrink-0 items-start justify-between gap-2 rounded-lg bg-warning/15 p-2.5 text-[11px] text-warning">
